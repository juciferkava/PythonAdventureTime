...
class character:
    ...
    def __init__(self, name, class_name, level, race, hitdice, age, proficiency):
        ...
        self.hp += add_HP(hp = self.hp, levels = (level ), con=self.abilityModifiers.con)
        self.current_hp = self.hp
        self.AC = 10 + self.abilityModifiers.dex

    def add_levels(self, levels):
        self.level += levels
        self.hp = add_HP(self.hp, levels=levels, con=self.abilityModifiers.con)
    #
